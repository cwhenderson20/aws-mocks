...
export default class SQS {
	...
	sendMessage(params, callback) {
		...
		connect(params.QueueUrl, (err, queue) => {
			...
			try {
				stringifiedBody = JSON.stringify(params.MessageBody);
			} catch (stringifyErr) {
				return callback(stringifyErr);
			}

			queue.add(stringifiedBody, { delay: params.DelaySeconds || 0 }, (addErr, id) => {
				if (addErr) {
					return callback(addErr);
				}

				callback(null, {
					MD5OfMessageBody: md5(stringifiedBody),
					MessageId: id
				});
			});
		});
